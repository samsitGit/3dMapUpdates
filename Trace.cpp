...
Trace::Trace(const std::string& directory) {
	...
	this->pcdDirectory=directory+"/pcd";
	this->poses = this->fetchPoses(directory + "/pose.txt");
}

pcl::PointCloud<pcl::PointXYZ>::Ptr Trace::loadFrame(int frame) const{
	std::string file_path = directory + "/" + std::to_string(frame) + ".pcd";
	return loadPointCloud(file_path);
}

std::vector<Eigen::Matrix4f> Trace::fetchPoses(const std::string& file_path) const {
